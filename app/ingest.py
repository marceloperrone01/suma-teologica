"""Embed chunks with BAAI/bge-m3 and persist to ChromaDB.

Run after data/chunks.jsonl has been produced by app.chunker.
"""
...
def make_model(model_name: str = EMBEDDING_MODEL) -> SentenceTransformer:
    ...
    if device == "cuda":
        model = model.half()  # FP16 ≈ 2.3 GB VRAM for bge-m3
    return model
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Some articles are duplicated in the source PDF (e.g. I-II q.101 a.1).
    # Dedupe by chunk_id, keeping the first occurrence.
    ...
    if not args.recreate and collection.count() > 0:
        # Skip chunks already indexed (resume support).
                ...
                metadatas=[
                    {
                        ...
                        "secao": c["secao"],
                        "citacao": c["citacao"],
                        "titulo_questao": c["titulo_questao"],
                        "titulo_artigo": c["titulo_artigo"],
                    }
                    for c in batch
                ],
            )
    ...
    print(f"Done. Collection has {collection.count()} chunks.", file=sys.stderr)
    return 0
